...
impl<Type: ScalarType> PolyRq<Type> {
    ...
    fn is_valid_data(context: &PolyContext<Type>, data: &Array2d<Type>) -> bool {
        for (rns_index, &ref modulus) in context.moduli.iter().enumerate() {
    ...
    pub fn coefficient(&self, coeff_index: usize) -> Vec<Type> {
        self.data.columns_iter().map(|row| row[coeff_index].clone()).collect::<Vec<_>>()
    }
    ...
    fn add_assign(&mut self, rhs: Self) {
        ...
        for (rns_index, modulus) in self.context.moduli.iter().enumerate() {
            for coeff_index in self.coeff_indices() {
                let lhs_value = self.data[rns_index][coeff_index].clone();
                let rhs_value = rhs.data[rns_index][coeff_index].clone();
                self.data[rns_index][coeff_index] = (lhs_value + rhs_value) % modulus.clone();
            }
        }
    }
}
    ...
    fn sub_assign(&mut self, rhs: Self) {
        ...
        for (rns_index, modulus) in self.context.moduli.iter().enumerate() {
            for coeff_index in self.coeff_indices() {
                let lhs_value = self.data[rns_index][coeff_index].clone();
                let rhs_value = rhs.data[rns_index][coeff_index].clone();
                self.data[rns_index][coeff_index] = (lhs_value + modulus.clone() - rhs_value) % modulus.clone();
            }
        }
    }
}
